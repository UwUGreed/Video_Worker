...
def serve_file_over_http(file_path):
    ...
    class SingleFileRequestHandler(BaseHTTPRequestHandler):
        ...
        def _serve(self, *, send_body):
            ...
            activity["last_media_request_at"] = time.time()
            activity["media_request_count"] = int(activity.get("media_request_count") or 0) + 1
            try:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            hosted_video = {
                ...
                "activity": local_server.get("activity") or {},
                "probe": probe,
            }
        except Exception as exc:
            active_stack.close()
...
def hold_buffer_tunnel_for_fetch(platform, hosted_video, hold_seconds):
    ...
    print(f"Buffer {platform}: holding temporary video URL open for {hold_seconds}s after last media request: {public_url}")
    if hold_seconds <= 0:
        return

    activity = (hosted_video or {}).get("activity") or {}
    if not activity:
        time.sleep(hold_seconds)
        return

    while True:
        last_request_at = float(activity.get("last_media_request_at") or 0.0)
        elapsed = time.time() - last_request_at if last_request_at > 0 else 0.0
        remaining = hold_seconds - elapsed if last_request_at > 0 else hold_seconds
        if remaining <= 0:
            return
